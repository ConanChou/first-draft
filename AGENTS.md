# FirstDraft — Agent Orientation

## What this is

Writing-first personal site. One author. Markdown in local folder → one command → site ships.
...
|-------|--------|
| SSG | Astro 6 |
| Styling | Hand-written global CSS (warm minimal palette, dark mode via `prefers-color-scheme`) |
| Content source | local markdown folder, exposed as `src/content` symlink and managed by `link` |
...
- Default lang (`en`) carries no suffix; other langs get `.<code>` suffix.
- Translations share the same ID.

### Publishing flow (two paths, never cross)

**A. CLI:** `draft` → write in editor → `publish <id>`  
**B. Micropub:** write in Micropub-capable app → Publish action → local Micropub server → shells out to `publish --title "..."`

`publish` does: rename file, fill front matter, `link` → `build` → `deploy`.
...
- Everything in `scripts/` (draft, publish, link, build, deploy, micropub lifecycle wrapper)
...
- **Side-effecting scripts**: use a temp dir (`fs.mkdtempSync`) — never touch real source library or content/
...
| Node scripts (no extension) | `node --check` | `node --check scripts/draft scripts/publish scripts/micropub scripts/link` |
...
- Anything that requires the real source markdown library to exist
...
- [x] RSS feeds per language (`/feed.<lang>.xml`)
